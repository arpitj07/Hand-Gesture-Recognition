...
while True:
	...
	#output = cv2.drawContours(frame , hull , -1 , (0,255,0))

	crop = frame[100:300 , 100:300]
	blur = cv2.GaussianBlur(crop , (3,3) , 0)
	hsv = cv2.cvtColor(blur , cv2.COLOR_BGR2HSV)
	msk = cv2.inRange( hsv , np.array([2,0,0]) , np.array([20,255,255]))
	...
	image , contours , heir = cv2.findContours( thres.copy() , cv2.RETR_TREE , cv2.CHAIN_APPROX_SIMPLE)
	...
	cv2.imshow("tCountours" , all_image)
	if cv2.waitKey(1)==32 :
		break   
